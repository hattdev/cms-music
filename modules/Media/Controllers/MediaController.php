...
//            }
            ...
            if (!empty($files)) {
                foreach ($files as $file) {
                    if(env('APP_PREVIEW_MEDIA_LINK')){
                        $file->thumb_size = route('media.preview',['id'=>$file->id,'size'=>'thumb']);
                        $file->full_size = route('media.preview',['id'=>$file->id,'size'=>'full']);
                        $file->medium_size = route('media.preview',['id'=>$file->id,'size'=>'medium']);
                    }else{
        ...
        public function getFilesInfo(Request $request)
        {
            ...
            if(!empty($ids = $request->ids)){
                $rows = $this->media->whereIn('id',$ids)->get()->toArray();
                if(!empty($rows)){
                    foreach ($ids as $k=> $id){
                        $filtered = \Arr::where($rows, function ($value, $key) use ($id){
                            return $value['id']==$id;
                        });
                        $data[$k] = \Arr::first($filtered);
                    }
            ...
            if ($check) {
                try {
                    ...
                    $fileObj->sizes = [
                        ...
                        '150'     => route('media.preview',['id'=>$fileObj->id,'size'=>'thumb']),
                        '600'     => route('media.preview',['id'=>$fileObj->id,'size'=>'medium']),
                        '1024'    => route('media.preview',['id'=>$fileObj->id,'size'=>'large']),
                    ];
...
//            }
            $model = Media::query()->whereIn("id",$file_ids);
//            if (!Auth::user()->hasPermissionTo("media_manage")) {
//                $model->where('create_user', Auth::id());
//            }
            $files = $model->get();
