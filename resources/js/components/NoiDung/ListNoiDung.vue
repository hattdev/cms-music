            ...
            <div class="col-12">
                <div class="card mb-5">
                    <div class="card-header bg-info text-white">
                        <h3 class="text-center">Quản lý Nội dung {{name}}</h3>
                    </div>
                    <div class="card-body pt-0">
                        <hatt-datatable :content_type="content_type" :rows="rows" :columns="columns" :total_rows="total_rows" :queryParams="queryParams" :propRoute="propRoute" :showLoader="showLoader" :listActions="listActions" @update-query-params="updateQueryParams" @removePost="removePost" @removePosts="removePosts">
    ...
    export default {
        ...
        watch: {
            $route(to,from){
                if(to.params.type!=from.params.type){
                    this.content_type = to.params.type;
                }
            },
        ...
        methods: {
            ...
            fetchData() {
                ...
                var page_url = page_url || cmsHatt.route.content.search;
                axios({
                    method: 'post',
                    url: page_url,
                    data: {
                ...
                cmsHattApp.showConfirm({
                    ...
                    callback: function(result){
                        if(result){
                            me.isLoading = true;
                            $.ajax({
                                url:cmsHatt.route.content.delete,
                                type:'POST',
                                data:{
                                    ids:[row.id],
